...
import { SiElectron, SiGin, SiGithub, SiGo, SiHtml5, SiShadcnui } from "react-icons/si"
...
const projects = [
  ...
  {
    title: "QR Genius Pro",
    description: "a QR generator and scanner app with advanced features for professionals.",
    tech: [
      { name: "Next.js", icon: RiNextjsFill, color: "currentColor" },
      { name: "Supabase", icon: SiSupabase, color: "#3ECF8E" },
      { name: "TypeScript", icon: SiTypescript, color: "#3178C6" },
  ...
  },
  {
    ...
    description: "An Electron-based app to manage your touchscreen and prevent input issues on games like Roblox.",
    tech: [
  ...
  {
    title: "Weather API",
    description: "A weather API that automatically detects user's location and returns the weather.",
    tech: [
  ...
  {
    title: "kash",
    description: "A multipurpose discord bot with fun, transfer and utility commands.",
    tech: [
      { name: "Next.js", icon: RiNextjsFill, color: "currentColor" },
    ...
    <div className="min-h-screen w-full bg-background p-6 md:p-12 lg:p-24">
      ...
      >
        ...
        <h1 className="text-6xl font-black tracking-tighter md:text-8xl lg:text-9xl">
          Featured <span className="inline-block bg-linear-to-r from-foreground/40 to-foreground bg-clip-text pr-[0.1em] text-transparent italic">Projects</span>
        </h1>
      </motion.div>
      ...
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project, index) => (
          ...
          >
            <Card className="group relative flex h-full flex-col overflow-hidden rounded-[32px] border-border bg-card shadow-lg transition-all hover:shadow-2xl hover:-translate-y-2">
              <div className="relative aspect-[16/10] w-full overflow-hidden flex items-center justify-center">
                <div className={cn(
                  "relative transition-transform duration-700 group-hover:scale-110 flex items-center justify-center",
                  project.logoMode 
                    ? (project.rounded ? "aspect-square h-full rounded-full overflow-hidden" : "h-full w-full p-4") 
                    : "h-full w-full"
                )}>
                  <Image
                    src={project.image}
                ...
                <div className="absolute inset-0 bg-linear-to-t from-background/60 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
              </div>
                  ...
                  {project.tech.map((t) => {
                    ...
                    return (
                      <div 
                        ...
                        //@ts-ignore
                        style={{ "--brand-color": (t as any).color } as React.CSSProperties}
                      >
                        {isImageUrl ? (
                          ...
                          //@ts-ignore
                          <Icon className="text-2xl transition-all duration-300 group-hover/tech:scale-125" style={{ color: "var(--brand-color)" }} />
                        )}
                        <span className="absolute -top-10 scale-0 rounded-md bg-foreground px-2 py-1 text-[10px] font-bold text-background transition-all group-hover/tech:scale-100 whitespace-nowrap">
                          {t.name}
                        </span>
                      </div>
                    );
